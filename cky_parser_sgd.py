...
class batch_CKY_parser:
    ...
    def set_models(self, p0, expansion, emission, pcfg_split=None):
        ...
        self._cache_outside_transforms()
        self.outside_chart = None

    def _cache_outside_transforms(self):
        log_g_3d = self.log_G.view(self.Q, self.Q, self.Q)  # A, B, C
        self.log_g_3d = log_g_3d
        self.left_transform = log_g_3d.permute(1, 0, 2).contiguous().view(
            self.Q, self.Q * self.Q
        )   # B <- (A, C)
        self.right_transform = log_g_3d.permute(2, 0, 1).contiguous().view(
            self.Q, self.Q * self.Q
        )  # C <- (A, B)
    ...
    def marginal(
        ...
        span_posteriors = None
        if return_span_posteriors and not only_viterbi:
            logprob_list, span_posteriors = self.span_constituent_posteriors_autograd(
                sents,
                return_log=span_posteriors_log,
                exclude_length_one=exclude_length_one
            )
        else:
            run_inside = (not only_viterbi) or return_span_posteriors
            if run_inside:
        ...
        if return_span_posteriors and only_viterbi:
            raise ValueError("return_span_posteriors=True is incompatible with only_viterbi=True.")
        ...
        if return_span_posteriors:
            return logprob_list, vtree_list, vproduction_counter_dict_list, vlr_branches_list, span_posteriors
        return logprob_list, vtree_list, vproduction_counter_dict_list, vlr_branches_list

    def compute_inside_span_tensors(self, sents):
        """
        Inside pass that keeps each span height as a separate tensor.

        This avoids writing chart cells in-place, which lets autograd compute
        d log Z / d span_score for all chart cells in one reverse pass.
        """
        try:
        ...
        spans = [self.get_lexis_scores(sents)]
        for height in range(1, sent_len):
            num_spans = sent_len - height
            split_scores = []
            for left_height in range(height):
                right_height = height - left_height - 1
                right_start = left_height + 1
                left = spans[left_height][:num_spans]
                right = spans[right_height][right_start:right_start + num_spans]
                split_scores.append(left[..., None] + right[..., None, :])

            dot_temp_mat = torch.logsumexp(
                torch.stack(split_scores, dim=0),
                dim=0
            ).view(num_spans, batch_size, -1)
            filtered_kron_mat = self.log_G + dot_temp_mat[:, :, None, :]
            spans.append(torch.logsumexp(filtered_kron_mat, dim=-1))

        logprob_list = torch.logsumexp(spans[-1][0] + self.log_p0, dim=1)
        return logprob_list, spans

    @staticmethod
    def _log_prob_from_marginals(marginals):
        zeros = torch.zeros_like(marginals)
        safe_marginals = torch.where(marginals > 0, marginals, zeros)
        return torch.where(
            safe_marginals > 0,
            safe_marginals.log(),
            torch.full_like(safe_marginals, -torch.inf)
        )

    def span_constituent_posteriors_autograd(
            self,
            sents,
            return_log=False,
            exclude_length_one=False,
        ...
        with torch.enable_grad():
            logprob_list, spans = self.compute_inside_span_tensors(sents)
            span_marginals = torch.autograd.grad(
                logprob_list.sum(),
                tuple(spans),
        ...
        sent_len = self.this_sent_len
        batch_size = len(sents)
        pad_value = -torch.inf if return_log else 0.0
        span_matrix = torch.full(
            (batch_size, sent_len, sent_len, self.Q),
            pad_value,
            device=spans[0].device,
            dtype=spans[0].dtype
        )

        for height, marginals in enumerate(span_marginals):
            if exclude_length_one and height == 0:
                continue
            values = marginals
            if return_log:
                values = self._log_prob_from_marginals(values)

            num_spans = sent_len - height
            starts = torch.arange(num_spans, device=spans[0].device)
            ends = starts + height
            span_matrix[:, starts, ends] = values.permute(1, 0, 2)

        return logprob_list, span_matrix
    ...
    @staticmethod
    def _log_domain_matmul(log_mat, log_rhs):
        """
        Stable log-domain matrix multiply.
        log_mat: [out_dim, in_dim], log_rhs: [num_items, in_dim]
        returns: [num_items, out_dim]
        """
        return torch.logsumexp(log_rhs[:, None, :] + log_mat[None, :, :], dim=-1)

    def compute_outside_logspace(self):
        assert self.this_sent_len > 0, "must call inside pass first!"
        sent_len = self.this_sent_len
        batch_size = self.left_chart.shape[2]
        if self.left_transform is None or self.right_transform is None:
            self._cache_outside_transforms()

        outside_chart = torch.full_like(self.left_chart, -torch.inf)
        outside_chart[sent_len - 1, 0] = self.log_p0.unsqueeze(0).expand(batch_size, -1)
        left_transform = self.left_transform
        right_transform = self.right_transform

        for parent_height in range(sent_len - 1, 0, -1):
            num_parent_spans = sent_len - parent_height
            parent_outside = outside_chart[parent_height, 0:num_parent_spans]  # spans, batch, Q

            for left_height in range(parent_height):
                right_height = parent_height - left_height - 1
                right_start = left_height + 1

                left_inside = self.left_chart[left_height, 0:num_parent_spans]  # spans, batch, Q
                right_inside = self.left_chart[right_height, right_start:right_start + num_parent_spans]

                left_inputs = (
                    parent_outside[:, :, :, None] + right_inside[:, :, None, :]
                ).reshape(-1, self.Q * self.Q)  # spans*batch, A*C
                left_updates = self._log_domain_matmul(left_transform, left_inputs).view(
                    num_parent_spans, batch_size, self.Q
                )
                current_left = outside_chart[left_height, 0:num_parent_spans].clone()
                outside_chart[left_height, 0:num_parent_spans] = torch.logaddexp(
                    current_left, left_updates
                )

                right_inputs = (
                    parent_outside[:, :, :, None] + left_inside[:, :, None, :]
                ).reshape(-1, self.Q * self.Q)  # spans*batch, A*B
                right_updates = self._log_domain_matmul(right_transform, right_inputs).view(
                    num_parent_spans, batch_size, self.Q
                )
                current_right = outside_chart[
                    right_height, right_start:right_start + num_parent_spans
                ].clone()
                outside_chart[right_height, right_start:right_start + num_parent_spans] = torch.logaddexp(
                    current_right, right_updates
                )

        self.outside_chart = outside_chart
        return outside_chart

    def span_constituent_posteriors(self, return_log=False, exclude_length_one=False):
        """
        Returns labeled span posteriors with shape [batch, start, end_inclusive, state].
        """
        assert self.this_sent_len > 0, "must call inside pass first!"
        if self.outside_chart is None:
            self.compute_outside_logspace()

        sent_len = self.this_sent_len
        batch_size = self.left_chart.shape[2]

        log_z = torch.logsumexp(self.left_chart[sent_len - 1, 0] + self.log_p0, dim=1)
        labeled_log_post = self.left_chart + self.outside_chart - log_z[None, None, :, None]
        if exclude_length_one:
            labeled_log_post[0] = -torch.inf

        span_matrix = torch.full(
            (batch_size, sent_len, sent_len, self.Q),
            -torch.inf,
            device=labeled_log_post.device,
            dtype=labeled_log_post.dtype
        )
        for height in range(sent_len):
            num_spans = sent_len - height
            starts = torch.arange(num_spans, device=labeled_log_post.device)
            ends = starts + height
            span_matrix[:, starts, ends] = labeled_log_post[height, 0:num_spans].permute(1, 0, 2)
        if return_log:
            return span_matrix
        return span_matrix.exp()
    ...
    def get_lexis_scores(self, sent_embs):
        ...
        return full_lexis_probs

    def get_lexis_prob(self, sent_embs, left_chart):
        full_lexis_probs = self.get_lexis_scores(sent_embs)
